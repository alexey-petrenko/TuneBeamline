...
from accelerator_devices import AccElemCollection
...
class OrbitCorMatPlot:
    def __init__(self, get_vals_from_consys, send_vals_to_consys, **kwargs):
        ...
        self.bpm_col = AccElemCollection('bpm_coll', 'BPM', bline)
        self.cor_col = AccElemCollection('cor_col', 'corrector', bline)
    ...
    def plot_beamline(self, bline, axis, h):
        ...
        for elem_name, elem_par in bline['Beamline elements'].items():
            ...
            # axis.plot([s, s], [0, -1], lw=1, color='blue', alpha=0.5)
            # axis.text(s, -1, ", ".join(name), verticalalignment='bottom', horizontalalignment='right',
            #           color='blue', fontsize=9, rotation='vertical', alpha=0.5)

    def plot_orbit(self, orbit_x=np.zeros(16,), orbit_y=np.zeros(16,)):
        self.axides[0].plot(self.bpm_col.s_x[self.bpm_col.ind_x], orbit_x[self.bpm_col.ind_x], "--", marker="o", lw=3)
        self.axides[1].plot(self.bpm_col.s_x[self.bpm_col.ind_y], orbit_y[self.bpm_col.ind_y], "--", marker="o", lw=3)

    def plot_correctors(self, x_kick=np.zeros(19,), y_kick=np.zeros(17,)):
        self.axides[2].plot(self.cor_col.s_x[self.cor_col.ind_x], x_kick[self.cor_col.ind_x], " ", marker="o",
                            markersize=8, markeredgewidth=1, markeredgecolor="black", color="white", alpha=0.5)
        self.axides[3].plot(self.cor_col.s_y[self.cor_col.ind_y], y_kick[self.cor_col.ind_y], " ", marker="o",
                            markersize=8, markeredgewidth=1, markeredgecolor="black", color="white", alpha=0.5)
